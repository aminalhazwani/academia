							...
							{% if item.image == 'true' %}
								<a href="{{ item.url }}">
									<div class="articlePreview_picture">
										<span class="articlePreview_type articlePreview_type-white">{{ item.type }}</span>
										<img class="articlePreview_cover" src="{{ item.picture }}" alt="#">
									</div>
								</a>
							{% else %}
								<span class="articlePreview_type">{{ item.type }}</span>
							{% endif %}
							<a href="{{ item.url }}">
								<h6 class="articlePreview_title">{{ item.title }}</h6>
							</a>
							<p class="articlePreview_excerpt">{{ item.excerpt | safe }}</p>
						</div>
					</div>
				{% endfor %}
			</div>
	...
	<section class="u-bg-gray-dark u-padd-top u-padd-btm u-push-btm-double">
		<div class="l-container">
			...
			<div class="g-row g-row g-clear-2@sm g-clear-4@md">
				{%- for item in movie.related %}
					<div class="g g-12 g-6@sm g-3@md">
						<a class="movie_poster" href="{{ item.url }}">
							<img class="u-img-max" src="{{ item.poster }}" alt="#">
						</a>
						<a href="{{ item.url }}">
							<h2 class="movie_title-small u-c-white">{{ item.title }}</h2>
						</a>
						<p class="movie_plays">{{ item.plays }}</p>
					</div>
				{%- endfor %}
			</div>
		</div>
				...
				<div class="g g-12 g-6@md">
					<div class="podcast_item podcast_item-archive">
						...
						<div>
							...
							<a href="{{ item.url }}">
								<h6 class="podcast_title podcast_title-archive u-td-underline-hover">{{ item.title }}</h6>
							</a>
	...
	<section class="u-push-btm">
		<div class="l-container">
			<h4 class="u-ff-sans u-fz-sm u-tt-uppercase u-fw-600 u-ta-center u-push-btm">Upcoming Events</h4>
				{% for item in home.events %}
					<div class="g-row">
						{% if item.image == 'true' %}
							<div class="g g-12 g-4@sm">
								<a class="event" href="{{ item.url }}">
							...
							<div class="g g-12 g-8@sm">
								<a href="{{ item.url }}">
									<h6 class="u-td-underline-hover">{{ item.title }}</h6>
								</a>
